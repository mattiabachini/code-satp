bootstrap_multiclass_weighted_f1_plot <- function(data,
                                                  fraction_value = "25%",
                                                  title = NULL,
                                                  caption = NULL,
                                                  xtitle = NULL,
  ...
  compute_weighted_f1 <- function(truth, pred) {
    ...
    total_support <- length(truth)
    sum(f1_scores) / total_support
  }
  ...
  bootstrap_model_f1 <- function(df_model, n = n_boot) {
    bootstraps(df_model, times = n) |>
      mutate(f1 = map_dbl(splits, ~ {
        ...
        compute_weighted_f1(df_split$true_label, df_split$pred_label)
      }))
  }
