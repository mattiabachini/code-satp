...
I am Manny Teitelbaum. I am an associate professor at GW in the Political Science Department and the Elliott School. I want to acknowledge my caouthors Mohiddin Bacha Shaik and Shiva Sharma, both of whom could unfortunately not be here today. Mohiddin is a data scientist who was a student in our MS DATS program and Shiva is a Ph.D. student in Public Policy who also works as an evaluator at the World Bank. 

## Overview

- In comparative politics and international relations, we spend a long time hand coding event data
- Our idea was to take one dataset that we had spent a lot of time and resources coding to see how well LLMs could do at reproducing the human codings
...
- We decided to focus on LLMs with an encoder-only architecture because a) they designed to understand and classify text and b) because they have significant advantages in terms of cost, speed, privacy and reproducibility compared to decoder-only models like Claude or the GPT series of models 

## Data: South Asia Terrorism Portal (SATP)

- So here is one event from our dataset that illustrate what I mean when I say "nice labels" and "lexical" complexity
- Read part of it
...
- And that's because of the way the data themselves were produced, which is that a non-profit, the Institute for Conflict Management, hired a team to collect news articles and write a paragraph-length summary of individual conflict events

## Prootocols for Human Codings

So then to code these events we followed this protocol where (read from slide)...

## Classification Tasks

We had our LLMs attempt three classification tasks for this exercise: who was the perpetrator, what was the action that they took and what was the target of their action? These are very similar classification tasks to what you would find in a confict database like GTD or ACLED with the main difference being the lexical complexity of the event descriptions and that some of the labels are more specific to our research interests, like "surrender" (which I mentioned earlier) or an attack on a mining company because there was a lot of mining activity in India's Red Corridor. 

## Models Compared

Here are the models that we compared. Each of them has a slightly different size and architecture. These are mostly general purpose BERT-based modes but notably we also included the domain specific ConfliBERT model which Javier is going to talk about in a little bit. 
...
These are the results for the perpetrator tasks. As you can see this was relatively easy for all of them. Except for the Electra model, they only needed about a quarto of the training data or 2000 examples to get up to F1 scores of .95 or so. And, with the exception of Electra, there are not really meaningful differences between the models.

## Action Type

Now looking at the action type task, we see a bit more variation in the performance of the models. Although they all end up with pretty high F1 scores, at the25% mark, we can see that RoBERTA, ConfliBERT and XLNet are all doing pretty well, getting up to F1 scores of .9 or better while BERT, Electra and DistilBERT are all lagging behind.

## Model Performance and Test Support by Action Type Label

Now looking at the F1 Scores for the individual labels at 100% of the training data, we can see that there is some varion in how well the models do acros the various action type labels. So for example the model does really well with arrest, surrender and armed assault, but not quite as well with infrastructure attacks or abduction. The issue seems to have more to do with lexical complexity than scarcity of data or label imbalance. So for example, surrender has a lot fewer examples than arrest or armed assault but the F1 scores are still really high. Whereas infrastructure and bombing don't have fewer examples than surrender but the F1 scores are lower. And I did try some imbalance handling techniques to try improve the scores of these labels but it didn't work very well and I think that is just because things like infrastructure attacks and kidnappings can be described in many different ways whereas are not a lot of different synonyms for "surrender."

## Target Type

For the target type model, we see a similar pattern to the action type model where RoBERTA, ConfliBERT and XLNet are all doing pretty well at the 25% mark while BERT, Electra and DistilBERT are lagging behind but it is even more pronounced and the overall F1 scores are lower than for the action type model.
