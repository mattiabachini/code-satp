...
for political science to how best to deploy them for specific research
contexts. As encoder-based models have become more accessible,
researchers face practical questions about which approaches offer the
best balance of performance, cost, and implementation complexity. Many
of the most time-consuming tasks in political event coding, such as
identifying common event types or actor roles, involve conceptually
routine classification decisions that may be well-suited to fine-tuned
encoder models, even when they appear in bespoke datasets with
domain-specific taxonomies.

This paper examines the performance of encoder-based transformer models
on conflict-related classification tasks, comparing both general-purpose
and domain-specific approaches. We use a subset of event descriptions
from the South Asia Terrorism Portal (SATP), focusing on the Maoist
insurgency in India. Events were hand-coded by researchers using a
multi-label taxonomy capturing event type, actor behavior, and
modalities of violence. The labels vary in frequency and complexity,
ranging from common, lexically distinctive labels like killing to more
semantically ambiguous ones such as infrastructure attack. While the
dataset shares many structural features with other conflict datasets
(such as GTD or ICEWS), it also includes some relatively niche labels
that were specific to the interests of the researchers working on this
project.
...
always necessary for good results.
...
Our findings suggest that encoder-based transformer models offer
practical solutions for a wide range of political coding challenges,
with performance determined by both model selection and methodological
choices. While domain-specific models like ConfliBERT provide additional
accuracy, particularly when label definitions are subtle or data are
scarce, general-purpose models such as RoBERTa can achieve comparable
performance on many tasks while offering advantages in speed and
accessibility. We also demonstrate that targeted imbalance handling
strategies can substantially improve performance on rare but
analytically important categories. For researchers working with limited
computational resources or sensitive data, the combination of
open-source encoder models with appropriate methodological interventions
may offer the most practical path to effective automated annotation
systems.
...
\subsection*{Decoder-Only LLM Baseline
Results}\label{decoder-only-llm-baseline-results}
\addcontentsline{toc}{subsection}{Decoder-Only LLM Baseline Results}
