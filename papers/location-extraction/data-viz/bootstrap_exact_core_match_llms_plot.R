# Bootstrap Exact Core Match Visualization for LLM Location Extraction Models
...
all_results <- map_dfr(models, function(model_name) {
  ...
  # Get prediction column name (model_name_prediction)
  ...
  df_model <- df |>
    select(true_label = true_location, prediction = !!sym(pred_col)) |>
  ...
  metrics_file <- file.path(RESULTS_DIR, paste0(model_name, "_metrics.json"))
...
p <- ggplot(core_match_summary, aes(x = model_label, y = core_match_mean)) +
  ...
  labs(
    title = "Bootstrap Exact Core Match: LLMs",
...
# Save plot
output_file <- file.path(OUTPUT_DIR, "bootstrap_exact_core_match_llms.png")
ggsave(
  output_file,
  plot = p,
  width = 10,
