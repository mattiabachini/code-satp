# Bootstrap Exact Core Match Visualization for Seq2Seq Location Extraction Models
...
all_results <- map_dfr(models, function(model_name) {
  # Construct file path
  csv_file <- file.path(RESULTS_DIR, paste0("location_", model_name, "_predictions.csv"))
  ...
  metrics_file <- file.path(RESULTS_DIR, paste0("location_", model_name, "_metrics.json"))
...
p <- ggplot(core_match_summary, aes(x = model_label, y = core_match_mean)) +
  ...
  labs(
    title = "Bootstrap Exact Core Match: Seq2Seq Models",
...
# Save plot
output_file <- file.path(OUTPUT_DIR, "bootstrap_exact_core_match_seq2seq.png")
ggsave(
  output_file,
  plot = p,
  width = 10,
